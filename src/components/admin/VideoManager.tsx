...
export default function VideoManager() {
  ...
  const handleSave = async () => {
    ...
    if (!editing && !videoFile && !videoUrl) {
  toast({ title: "Upload file OR paste YouTube link", variant: "destructive" });
  return;
}
    setLoading(true);
    try {
      let video_url = videoUrl || editing?.video_url || "";
      ...
      if (editing) {
        await supabase
          ...
          .update({
            ...
            video_url,
            thumbnail_url,
            category, // Add category to the update as well
          })
          .eq("id", editing.id);
        ...
        await supabase.from("videos").insert({
          ...
          video_url,
          thumbnail_url,
          category, // Add category to the database
        });
  ...
  setVideoUrl(item.video_url || ""); // ✅ FIX
  setCategory(item.category || "");  // ✅ FIX
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent>
          ...
          <div className="space-y-3">
            ...
            {/* ✅ Category 👇 */}
            <Input
              placeholder="Category (e.g. Ads, Campaign)"
              value={category}
            ...
            <FileUpload
              ...
              currentUrl={editing?.thumbnail_url || undefined}
              onFileSelected={setThumbFile}
